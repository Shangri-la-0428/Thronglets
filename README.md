...
现在这条线已经开始向 `Density Substrate` 迈一步：显式 signal 的机器结果里会直接带 `density_score` 和 `density_tier=sparse|candidate|promoted|dominant`，让“局部正在形成共识”不只是排序靠前，而是变成一个可读、可比较的状态。
